...
var statuses$4 = status$1;
status$1.message = codes$1;
status$1.code = createMessageToStatusCodeMap$1(codes$1);
status$1.codes = createStatusCodeList$1(codes$1);
status$1.redirect = {
  300: true,
  301: true,
  302: true,
...
function createMessageToStatusCodeMap$1(codes2) {
  var map = {};
...
function createStatusCodeList$1(codes2) {
  return Object.keys(codes2).map(function mapCode(code) {
    return Number(code);
  });
}
function getStatusCode$1(message) {
...
function getStatusMessage$1(code) {
...
function status$1(code) {
  if (typeof code === "number") {
    return getStatusMessage$1(code);
  }
  ...
  if (!isNaN(n)) {
    return getStatusMessage$1(n);
  }
  return getStatusCode$1(code);
}
  ...
  (function(module) {
    ...
    iconv.enableStreamingAPI = function enableStreamingAPI(stream_module2) {
      ...
      iconv.encodeStream = function encodeStream(encoding3, options) {
        return new iconv.IconvLiteEncoderStream(iconv.getEncoder(encoding3, options), options);
      };
      iconv.decodeStream = function decodeStream(encoding3, options) {
        return new iconv.IconvLiteDecoderStream(iconv.getDecoder(encoding3, options), options);
      };
...
function requireRawBody() {
  ...
  function getRawBody(stream3, options, callback) {
    ...
    return new Promise(function executor(resolve3, reject) {
      readStream(stream3, encoding3, length, limit2, function onRead(err, buf) {
...
function requireRead() {
  ...
  function read(req2, res2, next, parse2, debug2, options) {
    ...
    getBody(stream3, opts, function(error2, body) {
      ...
      try {
        ...
        req2.body = parse2(str, encoding3);
      } catch (err) {
  ...
  function contentstream(req2, debug2, inflate) {
    ...
    var stream3 = createDecompressionStream(encoding3, debug2);
    req2.pipe(stream3);
    return stream3;
  }
  function createDecompressionStream(encoding3, debug2) {
    switch (encoding3) {
      case "deflate":
        debug2("inflate body");
...
contentType$1.format = format$2;
contentType$1.parse = parse$a;
function format$2(obj) {
  ...
  var type3 = obj.type;
  if (!type3 || !TYPE_REGEXP$1.test(type3)) {
  ...
  if (parameters && typeof parameters === "object") {
    ...
    for (var i = 0; i < params.length; i++) {
      param2 = params[i];
      if (!TOKEN_REGEXP$1.test(param2)) {
        throw new TypeError("invalid parameter name");
      }
      string += "; " + param2 + "=" + qstring$1(parameters[param2]);
    }
  }
  return string;
}
function parse$a(string) {
  ...
  var header3 = typeof string === "object" ? getcontenttype(string) : string;
  if (typeof header3 !== "string") {
    throw new TypeError("argument string is required to be a string");
  }
  var index = header3.indexOf(";");
  var type3 = index !== -1 ? header3.slice(0, index).trim() : header3.trim();
  if (!TYPE_REGEXP$1.test(type3)) {
  ...
  if (index !== -1) {
    ...
    while (match3 = PARAM_REGEXP$1.exec(header3)) {
...
var mimeScore$3 = function mimeScore(mimeType, source = "default") {
  if (mimeType === "application/octet-stream") {
    return 0;
  }
  const [type3, subtype] = mimeType.split("/");
  const facet = subtype.replace(/(\.|x-).*/, "$1");
  const facetScore = FACET_SCORES$3[facet] || FACET_SCORES$3.default;
  const sourceScore = SOURCE_SCORES$3[source] || SOURCE_SCORES$3.default;
  const typeScore = TYPE_SCORES$3[type3] || TYPE_SCORES$3.default;
...
(function(exports) {
  ...
  function lookup2(path3) {
    ...
    var extension2 = extname2("x." + path3).toLowerCase().slice(1);
    if (!extension2) {
      return false;
    }
  ...
  function populateMaps(extensions, types) {
    Object.keys(db).forEach(function forEachMimeType(type3) {
      ...
      for (var i = 0; i < exts.length; i++) {
        ...
        types[extension2] = _preferredType(extension2, types[extension2], type3);
  ...
  function _preferredType(ext, type0, type1) {
    var score0 = type0 ? mimeScore6(type0, db[type0].source) : 0;
    var score1 = type1 ? mimeScore6(type1, db[type1].source) : 0;
...
function format$1(obj) {
  ...
  var type3 = obj.type;
  if (!type3 || !TYPE_NAME_REGEXP.test(type3)) {
    throw new TypeError("invalid type");
  }
...
function parse$9(string) {
  ...
  var match3 = TYPE_REGEXP.exec(string.toLowerCase());
  if (!match3) {
    throw new TypeError("invalid media type");
  }
  var type3 = match3[1];
  ...
  return new MediaType(type3, subtype, suffix);
}
function MediaType(type3, subtype, suffix) {
  this.type = type3;
 ...
 * MIT Licensed
 */
var contentType = contentType$1;
var mime$3 = mimeTypes$3;
...
function normalize$1(type3) {
  ...
  return type3.indexOf("/") === -1 ? mime$3.lookup(type3) : type3;
}
...
function normalizeType$1(value) {
  var type3 = contentType.parse(value).type;
  return typer.test(type3) ? type3 : null;
}
function tryNormalizeType(value) {
  try {
    return value ? normalizeType$1(value) : null;
  } catch (err) {
    return null;
  }
}
...
function requireUtils() {
  ...
  hasRequiredUtils = 1;
  var bytes2 = requireBytes();
  var contentType3 = contentType$1;
  ...
  function getCharset(req2) {
    try {
      return (contentType3.parse(req2).parameters.charset || "").toLowerCase();
    } catch {
      return void 0;
    }
  }
  ...
  function normalizeOptions(options, defaultType) {
    ...
    var limit2 = typeof (options == null ? void 0 : options.limit) !== "number" ? bytes2.parse((options == null ? void 0 : options.limit) || "100kb") : options == null ? void 0 : options.limit;
    var type3 = (options == null ? void 0 : options.type) || defaultType;
...
function requireJson() {
  ...
  function json2(options) {
    var { inflate, limit: limit2, verify, shouldParse } = normalizeOptions(options, "application/json");
  ...
  function firstchar(str) {
    var match3 = FIRST_CHAR_REGEXP.exec(str);
...
function requireRaw() {
  ...
  function raw(options) {
    var { inflate, limit: limit2, verify, shouldParse } = normalizeOptions(options, "application/octet-stream");
    function parse2(buf) {
      return buf;
    }
...
function requireText() {
  ...
  function text(options) {
    var { inflate, limit: limit2, verify, shouldParse } = normalizeOptions(options, "text/plain");
...
var decode$2 = function(str, defaultDecoder, charset3) {
...
var sentinel = {};
var stringify$3 = function stringify(object, prefix, generateArrayPrefix, commaRoundTrip, allowEmptyArrays, strictNullHandling, skipNulls, encodeDotInKeys, encoder, filter, sort, allowDots, serializeDate2, format2, formatter, encodeValuesOnly, charset3, sideChannel2) {
  var obj = object;
...
function requireUrlencoded() {
  ...
  function urlencoded(options) {
    var { inflate, limit: limit2, verify, shouldParse } = normalizeOptions(options, "application/x-www-form-urlencoded");
    var defaultCharset = (options == null ? void 0 : options.defaultCharset) || "utf-8";
    if (defaultCharset !== "utf-8" && defaultCharset !== "iso-8859-1") {
      throw new TypeError("option defaultCharset must be either utf-8 or iso-8859-1");
    }
    var queryparse = createQueryParser(options);
    function parse2(body, encoding3) {
      return body.length ? queryparse(body, encoding3) : {};
    }
    return function urlencodedParser(req2, res2, next) {
      ...
      var charset3 = getCharset(req2) || defaultCharset;
      if (charset3 !== "utf-8" && charset3 !== "iso-8859-1") {
        debug2("invalid charset");
        next(createError2(415, 'unsupported charset "' + charset3.toUpperCase() + '"', {
          charset: charset3,
          type: "charset.unsupported"
  ...
  function createQueryParser(options) {
    ...
    var charsetSentinel2 = options == null ? void 0 : options.charsetSentinel;
    var interpretNumericEntities2 = options == null ? void 0 : options.interpretNumericEntities;
    ...
    return function queryparse(body, encoding3) {
      ...
      try {
        return qs.parse(body, {
          ...
          arrayLimit,
          depth,
          charsetSentinel: charsetSentinel2,
          interpretNumericEntities: interpretNumericEntities2,
          charset: encoding3,
          parameterLimit,
          strictDepth: true
        });
  ...
  function parameterCount(body, limit2) {
    ...
    return len > limit2 ? void 0 : len - 1;
  }
  return urlencoded_1;
}
/*!
...
var mergeDescriptors_1 = mergeDescriptors;
...
function createHtmlDocument$1(message) {
  var body = escapeHtml$2(message).replaceAll("\n", "<br>").replaceAll("  ", " &nbsp;");
  return '<!DOCTYPE html>\n<html lang="en">\n<head>\n<meta charset="utf-8">\n<title>Error</title>\n</head>\n<body>\n<pre>' + body + "</pre>\n</body>\n</html>\n";
}
...
function finalhandler(req2, res2, options) {
  ...
  return function(err) {
    ...
    if (err) {
      ...
      msg = getErrorMessage(err, status3, env2);
    } else {
      status3 = 404;
    ...
    if (err && onerror) {
      setImmediate(onerror, err, req2, res2);
    }
    if (res2.headersSent) {
    ...
    send$2(req2, res2, status3, headers, msg);
  };
}
...
function getErrorMessage(err, status3, env2) {
  var msg;
  if (env2 !== "production") {
    msg = err.stack;
...
function send$2(req2, res2, status3, headers, message) {
  function write() {
...
var mimeScore$2 = function mimeScore2(mimeType, source = "default") {
  if (mimeType === "application/octet-stream") {
    return 0;
  }
  const [type3, subtype] = mimeType.split("/");
  const facet = subtype.replace(/(\.|x-).*/, "$1");
  const facetScore = FACET_SCORES$2[facet] || FACET_SCORES$2.default;
  const sourceScore = SOURCE_SCORES$2[source] || SOURCE_SCORES$2.default;
  const typeScore = TYPE_SCORES$2[type3] || TYPE_SCORES$2.default;
  const lengthScore = 1 - mimeType.length / 100;
  return facetScore + sourceScore + typeScore + lengthScore;
};
/*!
...
function forwarded$1(req2) {
  ...
  var proxyAddrs = parse$5(req2.headers["x-forwarded-for"] || "");
...
(function(module) {
  (function() {
    ...
    ipaddr2.IPv4.parser = function(string) {
      ...
      if (match3 = string.match(ipv4Regexes.fourOctet)) {
        return function() {
          var k, len, ref2, results;
          ref2 = match3.slice(1, 6);
          results = [];
      ...
      } else if (match3 = string.match(ipv4Regexes.longValue)) {
    ...
    ipaddr2.IPv6 = function() {
      ...
      IPv6.prototype.toRFC5952String = function() {
        var bestMatchIndex, bestMatchLength, match3, regex, string;
        regex = /((^|:)(0(:|$)){2,})/g;
        string = this.toNormalizedString();
        bestMatchIndex = 0;
        bestMatchLength = -1;
        while (match3 = regex.exec(string)) {
    ...
    ipaddr2.IPv6.parser = function(string) {
      var addr, k, len, match3, octet, octets, zoneId;
      if (ipv6Regexes["native"].test(string)) {
        return expandIPv6(string, 8);
      } else if (match3 = string.match(ipv6Regexes["transitional"])) {
        zoneId = match3[6] || "";
 ...
 */
(function(exports) {
  var { METHODS: METHODS2 } = require$$2$2;
  var contentType3 = contentType$1;
  var etag2 = etag_1;
  ...
  exports.normalizeType = function(type3) {
    return ~type3.indexOf("/") ? acceptParams(type3) : { value: mime2.lookup(type3) || "application/octet-stream", params: {} };
  };
  ...
  function acceptParams(str) {
    ...
    while (index < length) {
      ...
      var endIndex2 = colonIndex === -1 ? length : colonIndex;
      if (splitIndex > endIndex2) {
        index = str.lastIndexOf(";", splitIndex - 1) + 1;
        continue;
      }
      var key = str.slice(index, splitIndex).trim();
      var value = str.slice(splitIndex + 1, endIndex2).trim();
      if (key === "q") {
  ...
  exports.setCharset = function setCharset2(type3, charset3) {
    ...
    var parsed = contentType3.parse(type3);
    parsed.parameters.charset = charset3;
    return contentType3.format(parsed);
  };
  function createETagGenerator(options) {
    return function generateETag(body, encoding3) {
      var buf = !Buffer.isBuffer(body) ? Buffer.from(body, encoding3) : body;
...
var isPromiseExports = isPromise$3.exports;
var dist = {};
...
function* lexer(str) {
  ...
  while (i < chars.length) {
    const value = chars[i];
    const type3 = SIMPLE_TOKENS[value];
    if (type3) {
      yield { type: type3, index: i++, value };
    } else if (value === "\\") {
...
class Iter {
  ...
  consume(type3) {
    const value = this.tryConsume(type3);
    if (value !== void 0)
      return value;
    const { type: nextType, index } = this.peek();
    throw new TypeError(`Unexpected ${nextType} at ${index}, expected ${type3}: ${DEBUG_URL}`);
  }
  text() {
    let result = "";
    let value;
...
function compile(path3, options = {}) {
  ...
  const data = path3 instanceof TokenData ? path3 : parse$4(path3, options);
...
function pathToRegexp(path3, options = {}) {
  ...
  const items = paths.map((path4) => path4 instanceof TokenData ? path4 : parse$4(path4, options));
  for (const { tokens } of items) {
    for (const seq of flatten$2(tokens, 0, [])) {
  ...
  if (trailing)
    pattern += `(?:${escape$1(delimiter)}$)?`;
  pattern += end ? "$" : `(?=${escape$1(delimiter)}|$)`;
  const regexp = new RegExp(pattern, flags);
  return { regexp, keys };
}
function* flatten$2(tokens, index, init) {
  ...
  if (token.type === "group") {
    ...
    for (const seq of flatten$2(token.tokens, 0, fork)) {
      yield* flatten$2(tokens, index + 1, seq);
    }
  } else {
    init.push(token);
  }
  yield* flatten$2(tokens, index + 1, init);
}
...
function negate(delimiter, backtrack) {
  if (backtrack.length < 2) {
    if (delimiter.length < 2)
      return `[^${escape$1(delimiter + backtrack)}]`;
    return `(?:(?!${escape$1(delimiter)})[^${escape$1(backtrack)}])`;
  }
  if (delimiter.length < 2) {
    return `(?:(?!${escape$1(backtrack)})[^${escape$1(delimiter)}])`;
  }
  return `(?:(?!${escape$1(backtrack)}|${escape$1(delimiter)})[\\s\\S])`;
}
 ...
 * MIT Licensed
 */
const isPromise$1 = isPromiseExports;
const pathRegexp = dist;
const debug$3 = srcExports("router:layer");
const deprecate$1 = depd_1("router");
    ...
    if (_path instanceof RegExp) {
      ...
      return function regexpMatcher(p) {
        ...
        for (let i = 1; i < match3.length; i++) {
          const key = keys[i - 1];
          const prop = key.name;
          const val = decodeParam(match3[i]);
          if (val !== void 0) {
            params[prop] = val;
          }
        }
...
Layer$2.prototype.handleError = function handleError(error2, req2, res2, next) {
  const fn = this.handle;
  if (fn.length !== 4) {
    return next(error2);
  }
  try {
    const ret = fn(error2, req2, res2, next);
    if (isPromise$1(ret)) {
      if (!(ret instanceof Promise)) {
        deprecate$1("handlers that are Promise-like are deprecated, use a native Promise instead");
      }
...
Layer$2.prototype.handleRequest = function handleRequest(req2, res2, next) {
  const fn = this.handle;
  if (fn.length > 3) {
    return next();
  }
  try {
    const ret = fn(req2, res2, next);
    if (isPromise$1(ret)) {
      if (!(ret instanceof Promise)) {
        deprecate$1("handlers that are Promise-like are deprecated, use a native Promise instead");
      }
...
Layer$2.prototype.match = function match2(path3) {
  let match3;
  if (path3 != null) {
    if (this.slash) {
      this.params = {};
 ...
 * MIT Licensed
 */
const debug$2 = srcExports("router:route");
const Layer$1 = layer;
const { METHODS: METHODS$1 } = require$$2$2;
const slice$1 = Array.prototype.slice;
const flatten$1 = Array.prototype.flat;
const methods$1 = METHODS$1.map((method) => method.toLowerCase());
var route = Route$1;
...
Route$1.prototype._handlesMethod = function _handlesMethod(method) {
...
Route$1.prototype._methods = function _methods() {
...
Route$1.prototype.dispatch = function dispatch(req2, res2, done) {
  ...
  function next(err) {
    ...
    while (match3 !== true && idx < stack.length) {
      layer2 = stack[idx++];
      match3 = !layer2.method || layer2.method === method;
    }
    if (match3 !== true) {
      return done(err);
    }
    if (err) {
      layer2.handleError(err, req2, res2, next);
    } else {
      layer2.handleRequest(req2, res2, next);
    }
    sync = 0;
  }
};
Route$1.prototype.all = function all(handler) {
  const callbacks = flatten$1.call(slice$1.call(arguments), Infinity);
...
methods$1.forEach(function(method) {
  Route$1.prototype[method] = function(handler) {
    const callbacks = flatten$1.call(slice$1.call(arguments), Infinity);
    if (callbacks.length === 0) {
      throw new TypeError("argument handler is required");
    }
 ...
 * MIT Licensed
 */
const isPromise = isPromiseExports;
const Layer = layer;
const { METHODS } = require$$2$2;
const parseUrl = parseurlExports;
const Route = route;
const debug$1 = srcExports("router");
const deprecate = depd_1("router");
...
Router.prototype.handle = function handle(req2, res2, callback) {
  if (!callback) {
    throw new TypeError("argument callback is required");
  }
  debug$1("dispatching %s %s", req2.method, req2.url);
  let idx = 0;
  let methods2;
  ...
  if (req2.method === "OPTIONS") {
    methods2 = [];
    done = wrap(done, generateOptionsResponder(res2, methods2));
  }
  ...
  function next(err) {
    ...
    while (match3 !== true && idx < stack.length) {
      layer2 = stack[idx++];
      match3 = matchLayer(layer2, path3);
      route3 = layer2.route;
      ...
      const hasMethod = route3._handlesMethod(method);
      if (!hasMethod && method === "OPTIONS" && methods2) {
        methods2.push.apply(methods2, route3._methods());
      }
    ...
    processParams(self2.params, layer2, paramcalled, req2, res2, function(err2) {
      if (err2) {
        next(layerError || err2);
      } else if (route3) {
        layer2.handleRequest(req2, res2, next);
      } else {
        trimPrefix(layer2, layerError, layerPath, path3);
      }
      sync = 0;
    });
  }
  function trimPrefix(layer2, layerError, layerPath, path3) {
    if (layerPath.length !== 0) {
      ...
      debug$1("trim prefix (%s) from url %s", layerPath, req2.url);
      removed = layerPath;
    ...
    debug$1("%s %s : %s", layer2.name, layerPath, req2.originalUrl);
    if (layerError) {
      layer2.handleError(layerError, req2, res2, next);
    } else {
      layer2.handleRequest(req2, res2, next);
    }
  }
...
Router.prototype.use = function use(handler) {
  ...
  for (let i = 0; i < callbacks.length; i++) {
    ...
    debug$1("use %o %s", path3, fn.name || "<anonymous>");
    const layer2 = new Layer(path3, {
...
Router.prototype.route = function route2(path3) {
  const route3 = new Route(path3);
  const layer2 = new Layer(path3, {
    sensitive: this.caseSensitive,
    strict: this.strict,
    end: true
  }, handle2);
  function handle2(req2, res2, next) {
    route3.dispatch(req2, res2, next);
  }
  layer2.route = route3;
  this.stack.push(layer2);
  return route3;
...
methods.concat("all").forEach(function(method) {
  Router.prototype[method] = function(path3) {
    const route3 = this.route(path3);
    route3[method].apply(route3, slice.call(arguments, 1));
    return this;
  };
});
function generateOptionsResponder(res2, methods2) {
  return function onDone(fn, err) {
    if (err || methods2.length === 0) {
      return fn(err);
    }
    trySendOptionsResponse(res2, methods2, fn);
  };
}
...
function processParams(params, layer2, called, req2, res2, done) {
  ...
  function paramCallback(err) {
    ...
    if (!fn) return param2();
    try {
      const ret = fn(req2, res2, paramCallback, paramVal, key);
      if (isPromise(ret)) {
        if (!(ret instanceof Promise)) {
...
function sendOptionsResponse(res2, methods2) {
  const options = /* @__PURE__ */ Object.create(null);
  for (let i = 0; i < methods2.length; i++) {
    options[methods2[i]] = true;
  }
  const allow = Object.keys(options).sort().join(", ");
  res2.setHeader("Allow", allow);
  res2.setHeader("Content-Length", Buffer.byteLength(allow));
  res2.setHeader("Content-Type", "text/plain");
  res2.setHeader("X-Content-Type-Options", "nosniff");
  res2.end(allow);
}
function trySendOptionsResponse(res2, methods2, next) {
  try {
    sendOptionsResponse(res2, methods2);
  } catch (err) {
    next(err);
  }
}
...
(function(module, exports) {
  ...
  app2.handle = function handle2(req2, res2, callback) {
    ...
    Object.setPrototypeOf(res2, this.response);
    if (!res2.locals) {
      res2.locals = /* @__PURE__ */ Object.create(null);
    }
    this.router.handle(req2, res2, done);
  };
  app2.use = function use2(fn) {
    ...
    var fns = flatten2.call(slice2.call(arguments, offset), Infinity);
    if (fns.length === 0) {
    ...
    fns.forEach(function(fn2) {
      ...
      router2.use(path22, function mounted_app(req2, res2, next) {
        ...
        fn2.handle(req2, res2, function(err) {
          ...
          Object.setPrototypeOf(res2, orig.response);
          next(err);
        });
      });
      fn2.emit("mount", this);
...
function parseCharset(str, i) {
  var match3 = simpleCharsetRegExp.exec(str);
  if (!match3) return null;
  var charset3 = match3[1];
  var q = 1;
  if (match3[2]) {
...
function preferredCharsets$1(accept, provided) {
  ...
  var priorities = provided.map(function getPriority(type3, index) {
    return getCharsetPriority(type3, accepts2, index);
  });
...
function parseEncoding(str, i) {
  var match3 = simpleEncodingRegExp.exec(str);
  if (!match3) return null;
  var encoding3 = match3[1];
  var q = 1;
  if (match3[2]) {
    var params = match3[2].split(";");
    for (var j = 0; j < params.length; j++) {
...
function getEncodingPriority(encoding3, accepted, index) {
  var priority = { encoding: encoding3, o: -1, q: 0, s: 0 };
  for (var i = 0; i < accepted.length; i++) {
...
function preferredEncodings$1(accept, provided, preferred) {
  ...
  if (!provided) {
    return accepts2.filter(isQuality$2).sort(comparator).map(getFullEncoding);
  }
  var priorities = provided.map(function getPriority(type3, index) {
    return getEncodingPriority(type3, accepts2, index);
  });
  return priorities.filter(isQuality$2).sort(comparator).map(function getEncoding(priority) {
    return provided[priorities.indexOf(priority)];
  });
}
...
function parseLanguage(str, i) {
  var match3 = simpleLanguageRegExp.exec(str);
  if (!match3) return null;
  var prefix = match3[1];
  var suffix = match3[2];
  var full = prefix;
...
function preferredLanguages$1(accept, provided) {
  ...
  var priorities = provided.map(function getPriority(type3, index) {
    return getLanguagePriority(type3, accepts2, index);
  });
...
function parseMediaType(str, i) {
  var match3 = simpleMediaTypeRegExp.exec(str);
  if (!match3) return null;
...
function getMediaTypePriority(type3, accepted, index) {
  var priority = { o: -1, q: 0, s: 0 };
  for (var i = 0; i < accepted.length; i++) {
    var spec = specify(type3, accepted[i], index);
    if (spec && (priority.s - spec.s || priority.q - spec.q || priority.o - spec.o) < 0) {
      priority = spec;
    }
  }
...
function specify(type3, spec, index) {
...
function preferredMediaTypes$1(accept, provided) {
  ...
  var priorities = provided.map(function getPriority(type3, index) {
    return getMediaTypePriority(type3, accepts2, index);
  });
...
Negotiator$1.prototype.encoding = function encoding2(available, opts) {
  var set = this.encodings(available, opts);
  return set && set[0];
};
Negotiator$1.prototype.encodings = function encodings2(available, options) {
  var opts = options || {};
...
var mimeScore$1 = function mimeScore3(mimeType, source = "default") {
  if (mimeType === "application/octet-stream") {
    return 0;
  }
  const [type3, subtype] = mimeType.split("/");
  const facet = subtype.replace(/(\.|x-).*/, "$1");
  const facetScore = FACET_SCORES$1[facet] || FACET_SCORES$1.default;
  const sourceScore = SOURCE_SCORES$1[source] || SOURCE_SCORES$1.default;
  const typeScore = TYPE_SCORES$1[type3] || TYPE_SCORES$1.default;
  const lengthScore = 1 - mimeType.length / 100;
  return facetScore + sourceScore + typeScore + lengthScore;
};
/*!
...
function fresh$2(reqHeaders, resHeaders) {
  ...
  if (noneMatch) {
    ...
    var etag2 = resHeaders.etag;
    if (!etag2) {
      return false;
    }
    var matches = parseTokenList$1(noneMatch);
    for (var i = 0; i < matches.length; i++) {
      var match3 = matches[i];
      if (match3 === etag2 || match3 === "W/" + etag2 || "W/" + match3 === etag2) {
        return true;
      }
    }
    return false;
...
function mapWithIndex(range3, index) {
  return {
    start: range3.start,
    end: range3.end,
    index
...
req.range = function range2(size, options) {
  ...
  if (!range3) return;
  return parseRange$1(size, range3, options);
};
defineGetter(req, "query", function query() {
  ...
  var querystring = parse$3(this).query;
...
defineGetter(req, "protocol", function protocol() {
  ...
  var header3 = this.get("X-Forwarded-Proto") || proto;
  var index = header3.indexOf(",");
  return index !== -1 ? header3.substring(0, index).trim() : header3.trim();
});
...
contentDisposition$2.exports.parse = parse$2;
...
function decodefield(str) {
  ...
  var charset3 = match3[1].toLowerCase();
...
function parse$2(string) {
  ...
  var type3 = match3[1].toLowerCase();
  var key;
  var names = [];
  var params = {};
  var value;
  index = PARAM_REGEXP.lastIndex = match3[0].slice(-1) === ";" ? index - 1 : index;
...
var mimeScore4 = function mimeScore5(mimeType, source = "default") {
  if (mimeType === "application/octet-stream") {
    return 0;
  }
  const [type3, subtype] = mimeType.split("/");
  ...
  const typeScore = TYPE_SCORES[type3] || TYPE_SCORES.default;
  const lengthScore = 1 - mimeType.length / 100;
  return facetScore + sourceScore + typeScore + lengthScore;
};
/*!
...
(function(exports) {
  ...
  function _preferredType(ext, type0, type1) {
    var score0 = type0 ? mimeScore$12(type0, db[type0].source) : 0;
    var score1 = type1 ? mimeScore$12(type1, db[type1].source) : 0;
    return score0 > score1 ? type0 : type1;
  }
  function _preferredTypeLegacy(ext, type0, type1) {
    var _a;
...
SendStream.prototype.isPreconditionFailure = function isPreconditionFailure() {
  var req2 = this.req;
  var res2 = this.res;
  var match3 = req2.headers["if-match"];
  if (match3) {
    var etag2 = res2.getHeader("ETag");
...
SendStream.prototype.sendFile = function sendFile(path3) {
  ...
  fs.stat(path3, function onstat(err, stat) {
    ...
    if (err && err.code === "ENOENT" && !extname$1(path3) && !pathEndsWithSep) {
      return next(err);
    }
...
function clearHeaders(res2) {
  for (const header3 of res2.getHeaderNames()) {
    res2.removeHeader(header3);
  }
}
...
res.send = function send3(body) {
  ...
  if (chunk !== void 0) {
    if (Buffer.isBuffer(chunk)) {
      ...
      len = Buffer.byteLength(chunk, encoding3);
    } else {
  ...
  if (generateETag && len !== void 0) {
    if (etag2 = etagFn(chunk, encoding3)) {
      this.set("ETag", etag2);
    }
  }
...
res.contentType = res.type = function contentType2(type3) {
  var ct = type3.indexOf("/") === -1 ? mime.contentType(type3) || "application/octet-stream" : type3;
...
function initializeTokenPath() {
  tokenPath = path$3.join(app.getPath("userData"), "tokens.json");
}
...
function loadSavedTokens() {
  try {
    ...
    if (fs$2.existsSync(token_path)) {
      const tokenData = fs$2.readFileSync(token_path, "utf-8");
...
function streamToBuffer(stream22) {
  return new Promise((resolve3, reject) => {
    const chunks = [];
    stream22.on("data", (chunk) => chunks.push(chunk));
    stream22.on("end", () => resolve3(Buffer$2.concat(chunks)));
    stream22.on("error", reject);
  });
}
...
async function exportTextFeatureGDrive(content, filename, exportType2) {
  ...
  let buffer;
  let mimeType;
  filename = path$3.basename(filename, path$3.extname(filename));
  switch (exportType2) {
    case "md":
    case "txt":
      buffer = Buffer$2.from(content, "utf-8");
      mimeType = exportType2 === "md" ? "text/markdown" : "text/plain";
      filename += `.${exportType2}`;
      break;
    case "json":
      buffer = Buffer$2.from(JSON.stringify({ content }, null, 2), "utf-8");
    ...
    case "docx":
      {
        ...
        buffer = Buffer$2.from(docBuffer);
...
const __dirname = path$3.dirname(__filename);
let win = null;
...
try {
  ...
  console.warn("electron-acrylic-window not available:", error2);
}
app.name = "Calmeca";
initializeTokenPath();
function createWindow() {
  win = new BrowserWindow({
    ...
    icon: path$3.join(__dirname, "..", "assets", "taskbar.png"),
    webPreferences: {
  ...
  if (setVibrancy && win) {
    try {
      ...
      console.warn("Failed to apply vibrancy:", error2);
    }
  }
...
app.whenReady().then(() => {
  ...
  const PORT = 3e3;
  expressApp.use(express$1.static(path$3.join(__dirname, "..", "assets")));
  ...
  return new Promise(async (resolve3, reject) => {
    try {
      ...
      let authWindow = new BrowserWindow({
        ...
        icon: path$3.join(__dirname, "..", "assets", "taskbar.png"),
        webPreferences: {
          preload: path$3.join(__dirname, "..", "src", "preload.js"),
          contextIsolation: true,
          nodeIntegration: false
        }
      });
      ...
      authWindow.webContents.on("will-redirect", async (event, url2) => {
        if (!url2.startsWith(redirectUri)) return;
        event.preventDefault();
        const parsedUrl = new URL(url2);
        const code = parsedUrl.searchParams.get("code");
        const error2 = parsedUrl.searchParams.get("error");
        if (error2) {
        ...
        try {
          ...
          if (authWindow) {
            authWindow.loadFile(path$3.join(__dirname, "..", "assets", "oauth-redirect.html"));
          }
          setTimeout(() => {
            authWindow == null ? void 0 : authWindow.close();
            authWindow = null;
            resolve3({ success: true, tokens });
          }, 1e4);
        } catch (tokenError) {
      ...
      console.error("Failed to start Google login:", error2);
      reject(error2);
    }
  });
});
...
ipcMain.handle("drive-export-text", async (_event, args) => {
  try {
    const { content, filename, type: type3 } = args;
    const res2 = await exportTextFeatureGDrive(content, filename, type3);
    console.log("file exported: ", filename);
    return {
      success: true,
      fileId: res2.fileId,
      name: res2.name,
      driveUrl: res2.driveUrl
    };
  } catch (error2) {
...
ipcMain.handle("drive-import-file", async (_event, fileId) => {
  try {
    const res2 = await importDriveFile(fileId);
    console.log("Imported file:", res2.name);
    return {
      success: true,
      name: res2.name,
    ...
    console.log("Import error: ", error2);
    return { success: false, error: error2.message };
  }
});
